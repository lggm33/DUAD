...
@npc_bp.post("/<int:game_id>/npc/<int:npc_id>/convert-to-character")
@auth_required
def convert_npc_to_character(game_id: int, npc_id: int):
    """
    Convert an NPC to a player character.

    Only the DM can convert NPCs to characters.
    The new user must be an active member of the game.

    Request body:
    {
        "user_id": 123  // User who will own the new character
    }
    ...
    try:
        ...
        character = npc_service.convert_npc_to_character(
            npc_id=npc_id,
        ...
        if "only the dm" in error_msg:
            return error_response("FORBIDDEN", str(e), status_code=403)
        if "already converted" in error_msg:
            return error_response("CONFLICT", str(e), status_code=409)
        if "already has a character" in error_msg:
            return error_response("CONFLICT", str(e), status_code=409)
...
@npc_bp.post("/<int:game_id>/character/<int:character_id>/convert-to-npc")
@auth_required
def convert_character_to_npc(game_id: int, character_id: int):
    """
    Convert a player character to an NPC.

    Only the DM can convert characters to NPCs.
    The character must not already be converted.

    Request body (optional):
    {
        "npc_type": "COMPANION"  // Default: COMPANION
    }
    ...
    npc_type_str = data.get("npc_type", "COMPANION")
    npc_type = _parse_npc_type(npc_type_str)
    if npc_type is None:
        valid_types = [t.value for t in NPCType]
        return error_response(
    ...
    try:
        npc = npc_service.convert_character_to_npc(
